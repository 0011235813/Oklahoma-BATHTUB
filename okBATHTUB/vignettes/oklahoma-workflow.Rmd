...
```{r segment_plot, fig.height=5.5, eval=requireNamespace("ggplot2", quietly=TRUE) && exists("chain") && is.list(chain) && exists("chain")}
...
```{r response_plot, fig.height=5.5, eval=requireNamespace("ggplot2", quietly=TRUE) && exists("baseline") && exists("baseline")}
ok_plot_response(
  baseline,
  response     = "tsi",
  target_class = "mesotrophic",
...
```{r scenario_plot, fig.height=5, eval=requireNamespace("ggplot2", quietly=TRUE) && exists("sweep") && is.data.frame(sweep) && exists("sweep")}
...
```{r full_workflow}
# Arcadia Lake morphometry (hardcoded for reproducibility)
loading <- list(
  inflow_m3yr   = 45e6,
  tp_inflow_ugl = 120,
  tn_inflow_ugl = 1800
...
# Run pipeline with Oklahoma Cross Timbers coefficients
baseline_fw <- ok_load(
...
sweep_fw <- ok_scenario_sweep(baseline_fw, max_reduction_pct = 60,
                               step_pct = 10, target_class = "mesotrophic")
print(sweep_fw[, c("scenario", "tp_inflow_ugl", "tsi_mean",
                    "trophic_state", "meets_target")])
```

```{r full_workflow_plot, eval=requireNamespace("ggplot2", quietly=TRUE) && exists("baseline_fw")}
ok_plot_response(baseline_fw, response = "tsi",
                  target_class = "mesotrophic",
                  current_tp   = 120,
                  lake_name    = "Arcadia Lake")
```
