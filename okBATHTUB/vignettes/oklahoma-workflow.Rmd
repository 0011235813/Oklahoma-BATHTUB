...
knitr::opts_chunk$set(
  ...
  fig.height = 5,
  dpi        = 120,
  eval       = FALSE   # AWQMS connection not available in CRAN check env
)
```
...
obs_tsi <- arcadia_data |>
  filter(!is.na(tp_ugl), !is.na(chla_ugl), !is.na(secchi_m)) |>
  rowwise() |>
  mutate(
...
ok_segment_summary(chain) |>
  select(segment, tp_inflow_ugl, tp_inlake_ugl, tp_retention,
...
```{r segment_plot, fig.height=5.5, eval=requireNamespace("ggplot2", quietly=TRUE)}
...
sweep |>
  select(scenario, tp_inflow_ugl, tp_inlake_ugl,
...
custom |>
  select(scenario, tp_inflow_ugl, tsi_mean, trophic_state, meets_target) |>
  print()
```
...
```{r scenario_plot, fig.height=5, eval=requireNamespace("ggplot2", quietly=TRUE)}
...
all_tsi <- all_lakes |>
  filter(!is.na(tp_ugl), !is.na(chla_ugl)) |>
  mutate(
    tsi_tp   = 14.42 * log(tp_ugl)   + 4.15,
    tsi_chla = 9.81  * log(chla_ugl) + 30.6
  )
