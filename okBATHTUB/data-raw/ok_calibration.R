...
#
# This script fits the Oklahoma-specific empirical regression coefficients
# used by the okBATHTUB package's "oklahoma" coefficient set. It is shipped
# in data-raw/ for transparency and reproducibility, NOT as a script that
# runs automatically at install or load time.
#
# Author note: okBATHTUB is independent open-source research software
# developed by Jordon Henderson on personal time and personal equipment.
# It is not affiliated with or endorsed by any employer or government
# agency. The calibration uses publicly available Oklahoma lake
# monitoring data (Beneficial Use Monitoring Program / Lake Monitoring
# Program records, retrieved from AWQMS in a prior local analysis).
#
...
#
# TP retention uses Walker BATHTUB Model 1 (second-order available-P
# sedimentation) — see ok_retention() in the package. Retention
# calibration would require paired inflow load data and is out of scope
# here.
#
# Ecoregion calibration outcomes (per ok_calibration_report.xlsx):
#   Cross Timbers            -> ecoregion-specific (n=169 / 36 lakes)
#   Central OK/TX Plains     -> ecoregion-specific (n=24 / 10 lakes)
#   Ozark Highlands Chl-a    -> ecoregion-specific (n=20 / 14 lakes)
#   Ozark Highlands Secchi   -> statewide pooled (R^2=0.228 < 0.25)
#   Arkansas Valley          -> statewide pooled (R^2=0.120 Chl-a;
#                               0.018 Secchi; turbidity decouples
#                               TP -> Chl-a -> Secchi)
#   Ouachita Mountains       -> statewide pooled (n=10 < 15 threshold)
#   Central Great Plains     -> statewide pooled (n=1 below threshold)
#   All other ecoregions     -> statewide pooled (no in-state lakes)
#
# Minimum thresholds for ecoregion-specific fit: n >= 15 obs, >= 5 lakes,
# R^2 >= 0.25. Below threshold: use statewide pooled. If pooled fails
# too: Walker defaults (not triggered in current calibration).
#
# Outputs:
#   ok_calibration_report.xlsx -- full coefficient table with diagnostics
#                                  (THIS is the source of truth for the
#                                  values in .oklahoma_coefficients())
#   ok_calibration_plots/      -- diagnostic plots per ecoregion
#
# Dependencies: dplyr, tidyr, ggplot2, openxlsx, stringr, readr
# Requires: an `all_lakes` data frame in session with the columns
#   listed in data-raw/CALIBRATION_README.md (Recalibration section).
# =============================================================================
...
# Provide local definitions of utility helpers that were originally
# sourced from the AWQMS connector (which is not part of the public
# package). Users adapting this script for recalibration can replace
# ok_path() with their own output directory logic.
ok_path <- function(...) {
  base <- Sys.getenv("OKBATHTUB_DATARAW",
